...
function Search() {
    ...
    const getSearchTracks = () => {
        spotifyApi.searchTracks(`track:${searchQuery}`).then((data) => {
    ...
    const getSearchAlbums = () => {
        spotifyApi.searchAlbums(`album:${searchQuery}`).then((data) => {
            setAlbums(data.body.albums?.items)
            setAlbumIds(data.body.albums?.items.map((album) => {
                return album.id
            }))
        }).catch((err) => {
            console.log(err);
    ...
    const getSearchArtists = () => {
        spotifyApi.searchArtists(`artist:${searchQuery}`).then((data) => {
            setArtists(data.body.artists?.items)
            setArtistIds(data.body.artists?.items.map((artist) => {
                return artist.id
            }))
        }).catch((err) => {
            console.log(err);
    ...
    return (
        <Layout>
            <div>
                {/* Lib center */}
                <div className="py-24">
                    <h1 className='text-lg md:text-xl xl:text-2xl text-white'>ARTIST Results</h1>
                    ...
                    <h1 className='text-lg md:text-xl xl:text-2xl text-white'>ALBUM Results</h1>
                    <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 pt-3' >
                        {
                            albums?.map((album) => (
